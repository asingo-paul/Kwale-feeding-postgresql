...
def dashboard():
    ...
    cur.execute("""
        SELECT d.distribution_date, d.location, d.beneficiary_count, 
               STRING_AGG(CONCAT(fi.name, ' (', di.quantity, ' ', fi.unit, ')'), ', ') as food_items
...
def add_fund():
    if request.method == 'POST':
        ...
        try:
            ...
            if not donor:
                cur.execute("INSERT INTO donors (name) VALUES (%s) RETURNING id", (donor_name,))
...
def distributions():
    ...
    # Get stats
    cur.execute("SELECT COUNT(*) as count FROM distributions WHERE distribution_date >= CURRENT_DATE - INTERVAL '1 month'")
    monthly_distributions = cur.fetchone()['count']
    
    cur.execute("""
        SELECT SUM(beneficiary_count) as total 
        FROM distributions 
        WHERE distribution_date >= CURRENT_DATE - INTERVAL '1 month'
    """)
    ...
    cur.execute("""
        ...
        WHERE d.distribution_date >= CURRENT_DATE - INTERVAL '1 month'
    """)
...
def add_distribution():
    if request.method == 'POST':
        ...
        try:
            ...
            distribution_id = cur.fetchone()['id']
...
def reports():
    ...
    cur.execute("""
        ...
        FROM funds 
        WHERE received_date >= CURRENT_DATE - INTERVAL '6 months'
    ...
    cur.execute("""
        SELECT TO_CHAR(distribution_date, 'YYYY-MM') as month, 
               SUM(beneficiary_count) as beneficiaries_served
        FROM distributions 
        WHERE distribution_date >= CURRENT_DATE - INTERVAL '6 months'
        GROUP BY TO_CHAR(distribution_date, 'YYYY-MM')
        ORDER BY month
    """)
